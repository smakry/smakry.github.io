...
- Go的宕机机制让`延迟执行的函数`在栈清理之前调用（可用延迟函数转移堆栈信息用于排查）；defer panic recover构成异常处理机制。`利用recover处理panic指令，defer必须在panic之前声明，否则当panic时，recover无法捕获到panic`    
