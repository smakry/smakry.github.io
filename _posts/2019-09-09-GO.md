...
- 优雅关闭channel
1. 一个sender，一个receiver （直接在sender端关闭即可）
2. 一个sender，多个receiver （直接在sender端关闭即可）
3. 多个sender，一个receiver 
    - 增加一个传递`关闭信号的channel`，receiver通过信号channel下达关闭数据channel指令，sender端监听到关闭信号，停止发送数据
    - 利用GC优雅的回收channel（非关闭信号的channel）
```go
func main() {
...
```
4. 多个sender，多个receiver（由一个中间的close来管理，发送跟接收都听从这个中间人）
```go
func main() {
    rand.Seed(time.Now().UnixNano())
...
- goroutine与操作系统线程的差别（https://github.com/aceld/golang/blob/main/2%E3%80%81Golang%E7%9A%84%E5%8D%8F%E7%A8%8B%E8%B0%83%E5%BA%A6%E5%99%A8%E5%8E%9F%E7%90%86%E5%8F%8AGMP%E8%AE%BE%E8%AE%A1%E6%80%9D%E6%83%B3%EF%BC%9F.md）
    - **栈增长方式**：
...
#### runtime.Gosched 让出cpu时间片，默认情况是单procs运行，cpu切换执行。
```go
func say(s string) {
    for i := 0; i < 2; i++ {
        runtime.Gosched()
