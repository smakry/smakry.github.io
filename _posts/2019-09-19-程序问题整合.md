...
new[]时多分配了4个字节（存储实际分配的对象个数，`但内置类型或者无自定义的析构函数的类类型是不会多多4个字节`），new[]内嵌类型使用delete正常（内嵌类型不会向后偏移4个字节，此时使用delete可以正常找到存在对象的个数大小），但自定义类型使用delete时会偏移4个字节导致析构异常。  
不管哪种情况，`配对是正确的选择`。    
...
String& String::operator=(const String& rhs){
    if(this != &rhs){ //当与当前的已经一致了就不需要再赋值了
        ...
        m_data = new char[strlen(rhs.Get()) + 1];
...
- 基类与子类之间的转换：`子类转成基类安全`的（多态中的应用及这种转化）；基类转成子类的不安全的（并没有子类的成员，因为基类没有子类的成员无法赋值，`但是成员方法还是有`的）；基类与子类之间的转换用dynamic_cast比较好  
